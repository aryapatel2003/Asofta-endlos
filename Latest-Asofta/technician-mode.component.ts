...
export class TechnicianModeComponent implements OnInit,OnDestroy {
  ...
  public currentErrors : Array<MachineErrorTypeView> = new Array<MachineErrorTypeView>();
  ...
  constructor(
    ...
    this.socketMessage = this.webSocketService.getSocketMessage.subscribe((response : SocketResponseView)  => {
      ...
      if(response.socketCommand == 1){
        ...
        if(response.maintenanceModeResponseView != null){
          this.currentErrors = response.maintenanceModeResponseView.currentErrors;
          this.machineView = response.maintenanceModeResponseView.machineView;
          this.machineCapacityView = response.maintenanceModeResponseView.machineCapacityView || new MachineCapacityView();
          this.machineSensorView = response.maintenanceModeResponseView.machineSensorView || new MachineSensorView();
        ...
        this.passwordIncorrectPopup = true;
         ...
         this.sensorDataModels = response.maintenanceModeResponseView.sensorDataModels;
      }
      // else if(response.socketCommand == 9){
      //   window.location.reload();
      // }
    });
    this.socketConnectionStatus = this.webSocketService.getConnectionStatus.subscribe(status => {
  ...
  appendToInput(key: string) {
    if (this.activeInput && this.activeInput.startsWith('capacity')) {
      ...
      this.machineCapacityView[inputField] = (this.machineCapacityView[inputField] || '') + key;
      ...
      this.machineSensorView[inputField] = (this.machineSensorView[inputField] || '') + key;
    }
  }
  ...
  backspace() {
    if (this.activeInput && this.activeInput.startsWith('capacity')) {
      const inputField = this.activeInput.substring('capacity'.length); // Extract field name from activeInput
      this.machineCapacityView[inputField] = this.machineCapacityView[inputField].slice(0, -1);
    } else if (this.activeInput && this.activeInput.startsWith('sensor')) {
      const inputField = this.activeInput.substring('sensor'.length); // Extract field name from activeInput
      this.machineSensorView[inputField] = this.machineSensorView[inputField].slice(0, -1);
    }
  }
